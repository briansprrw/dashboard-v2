...
|---|---|---|---|---|---|
| M0 | [Product and architecture decisions](./M0-product-and-architecture-decisions.md) | Not Started | Opus 4.8 `xhigh` | Codex + Brian | Brian |
| M1 | [Foundation and delivery](./M1-foundation-and-delivery.md) | Not Started | Sonnet 5 `high` | Opus 4.8 `high` + Codex | Brian |
...
| M3 | [Glance dashboard and core tasks](./M3-glance-dashboard-and-core-tasks.md) | Not Started | Sonnet 5 `high` | Opus 4.8 `high` + Codex visual/functional QA | Brian |
...
| M6 | [Migration and reconciliation](./M6-migration-and-reconciliation.md) | Not Started | Fable 5 `high` or Opus 4.8 `xhigh` | independent Opus 4.8 `xhigh` + Codex | Brian |
| M7 | [Hardening and device QA](./M7-hardening-and-device-qa.md) | Not Started | Opus 4.8 `xhigh` | Codex; targeted human device QA | Brian |
| M8 | [Production launch](./M8-production-launch.md) | Not Started | Opus 4.8 `high` as runbook assistant | Codex + Brian at every production gate | Brian |
...
M6 read-only source profiling and synthetic-fixture design may begin after M0. Import implementation must wait until the destination schema is stable. No production source access or write freeze is implied by that overlap.
...
The default implementation model is **Claude Sonnet 5 at `high` effort**: it offers the best speed/intelligence balance for scoped coding and test iteration. Use **Claude Opus 4.8 at `xhigh`** where a wrong answer could reshape the architecture, break authorization, expose private data, or corrupt migration. Use **Claude Fable 5 at `high`** only for genuinely long-running cross-system work such as a full migration rehearsal; it is not needed for ordinary work packets. **Claude Haiku 4.5** is allowed for mechanical, easily checked inventory only and has no effort control.
