...
**Commit / diff base:** `05ae778a84a18281a2e7dc15e0224fe3b04b02cd` (uncommitted M3.1–M3.6 worktree on top).
...
retained per AC-G2, and the `complete` row's opacity). The mockup's search, filter
chips, FAB, and Legend bar are **not** present in these captures — they are new
functionality rather than restyling (see M3.6-D3).
...
| VP-8     | 1280×800   | `vp8-smart-frame-1280x800-glance-dark-normal.png`           | Glance / dark | Normal                                       | Smart-frame (small). No-hover distinction is structural, not emulated — see Known limitations. |
...
## Min-yield column fallback (AC-G5: "max is firm; min may yield... fallback must be visible in M3 evidence")

| File                                                       | Viewport | Config                           | Result                                                                                                                                                |
| ---------------------------------------------------------- | -------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `vp1-narrow-420x1080-standard-dark-min-yield-fallback.png` | 420×1080 | `columnBounds: {min: 3, max: 3}` | Grid yields to 1 column (confirmed via `getComputedStyle` track count in `min-yield.spec.ts`) rather than forcing an unsafe 3-column reflow at 420px. |
| `vp7-tablet-1180x820-standard-dark-min-honored.png`        | 1180×820 | `columnBounds: {min: 2, max: 3}` | Grid honors the min (≥2 columns) once the viewport is wide enough.                                                                                    |
...
- **Opus UI/state review:** Out of this packet's scope — flagged for a separate reviewer pass per the M3 milestone doc's Review model row.
